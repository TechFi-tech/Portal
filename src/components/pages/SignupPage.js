...
class SignupPage extends React.Component {
    ...
    handleSignup(e) {
        ...
        this.setState((state) => {
            ...
            let {name, email, plan, errors} = this.state;
            const hasFormErrors = (errors && Object.values(errors).filter(d => !!d).length > 0);
            if (!hasFormErrors) {
                // fetch('http://localhost:8000/api/v1.0/members', {
                //     method: 'POST',
    ...
    handleChooseSignup(e, plan) {
        ...
        this.setState((state) => {
            ...
            const hasFormErrors = (errors && Object.values(errors).filter(d => !!d).length > 0);
            if (!hasFormErrors) {
                onAction('signup', {name, email, address, plan});
                this.setState({
                    errors: {}
                });
            }
            ...
            <section>
                <div className='gh-portal-section'>
                    <div className='gh-portal-logged-out-form-container'>
                        ...
                        />
                        {/* <p className='gh-portal-input-label'>My address</p>
                        <p className='gh-portal-input'>{this.state.address}</p>
                        <button className='gh-button-connect-wallet' onClick={() => this.connectWallet()}>
                            Connect Wallet
                        </button> */}
                    </div>
                    <div>
