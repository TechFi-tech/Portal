...
import httpGet from '../../utils/fetchData';
...
const UserHeader = ({avatarImg}) => {
    const {member, brandColor} = useContext(AppContext);
    const avatar = avatarImg ? avatarImg : member.avatar_image;
    // const avatar = member.avatar_img;
    return (
...
const AccountActions = () => {
    ...
    // const {name, address, email, subscribed} = member;
    ...
    return (
        <div>
            <a href='/profile' target='_blank' rel='noopener noreferrer' className="no_decoration gh-portal-btn margin-bottom-12" onClick={() => {
...
const AccountMain = () => {
    const jwtToken = window.localStorage.getItem('INTEGRATION_JWT_MEMBER');
    // const jwtToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZGVudGlmeSI6IjYyODRjOWUzYjA0YzRmMzVhYjNlNWY2ZCIsInJvbGVzIjpbInVzZXIiXSwibWV0YURhdGEiOnsiZW1haWwiOiJ0aGFuaG52emVAZ21haWwuY29tIn0sImlhdCI6MTY1MzAzMDg3NiwiZXhwIjoxNjY4NTgyODc2fQ.GSkgHYAVWnlAbZK_EjAVaW0epvmKPrStoRiFV79L-rw';
    const userProfile = httpGet(`https://integration.techfi.tech/staging/api/v1.0/users/profiles`, {}, {Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: 'Bearer ' + jwtToken});
    let avatarImg = '';
    try {
        const userData = JSON.parse(userProfile);
        avatarImg = userData.profile.profile_image;
    } catch {
        avatarImg = null;
    }
    ...
    return (
        <div className='gh-portal-content gh-portal-account-main'>
            <CloseButton />
            <UserHeader avatarImg={avatarImg}/>
            {/* <UserHeader /> */}
